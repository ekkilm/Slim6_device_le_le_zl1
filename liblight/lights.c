...
static char*
get_scaled_duty_pcts(int brightness, char *buf, int buflen)
{
	int bufpos = 0;
    int i = 0;
    ...
    for (i = 0; i < RAMP_SIZE; i++) {
        ...
        bufpos += snprintf(&buf[bufpos], buflen - bufpos - 1, "%d", (BRIGHTNESS_RAMP[i] * brightness / 255));
    }
	buf[bufpos] = 0;
    ALOGV("%s: brightness=%d bufpos=%d duty=[%s]", __func__, brightness, bufpos, buf);
    return buf;
}
        ...
        struct light_state_t const* state)
{
    ...
    if (blink) {
		char dutyBuf[5 * RAMP_SIZE * sizeof(char)];
        ...
        duty = get_scaled_duty_pcts(red, dutyBuf, sizeof(dutyBuf));    
        ...
        duty = get_scaled_duty_pcts(green, dutyBuf, sizeof(dutyBuf));
        ...
        duty = get_scaled_duty_pcts(blue, dutyBuf, sizeof(dutyBuf));
